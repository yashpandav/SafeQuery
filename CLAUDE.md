...
│   ├── vault-client/     # HashiCorp Vault database secrets engine HTTP client ✅ BUILT
...
├── .github/
│   └── workflows/        # ci.yml (check-types + lint + test) + docker.yml (image builds) ✅ BUILT
├── CLAUDE.md             # This file
├── turbo.json
├── pnpm-workspace.yaml
└── package.json
...
HashiCorp Vault database secrets engine HTTP client (SQ-044). Uses native `fetch` (Node 22 — no
additional dependencies). `createVaultDatabaseClient({ addr, token })` returns a client with four
methods: `registerConnection(params)` — enables the database secrets engine mount (idempotent),
configures the Postgres admin connection, and creates a short-TTL read role (5 min) and a shorter
write role (2 min); `mintCredentials(connectionId, 'read'|'write')` — calls
`GET /v1/database/creds/{role}` and returns `{ username, password, leaseId, leaseDuration }`;
`revokeLease(leaseId)` — calls `PUT /v1/sys/leases/revoke` for explicit post-execution cleanup;
`deregisterConnection(connectionId)` — removes both roles and the connection config (best-effort,
errors ignored so cleanup never blocks deletion). `VaultCredentialRef = { type: 'vault', connectionId }`
is the JSON stored in `database_connections.encrypted_credentials` when Vault is configured — executor
detects this shape at runtime and mints instead of decrypting. Both paths coexist: connections enrolled
before Vault was configured continue to decrypt via `CREDENTIAL_MASTER_KEY`. 11 unit tests
(`pnpm --filter @repo/vault-client test`) mock `fetch` globally via `vi.stubGlobal` covering
mount idempotency, SSL mode, error propagation, lease revocation, and best-effort deregister.
...
work for a job nobody can attribute to an org. `TestConnectionJobData` also carries `connectionId`
(pre-generated by `apps/api` via `crypto.randomUUID()`) so `apps/tre-executor` can use it as the
stable Vault config/role name and `apps/api` can insert the `database_connections` row with the same
UUID — ensuring the Vault path name and the DB row ID are always in sync.
...
- **`handleTestConnection`** — connects with the raw plaintext credentials from the job payload, runs
  `SELECT 1`. On success: if `VAULT_ADDR`/`VAULT_TOKEN` are set, calls `vaultClient.registerConnection()`
  to enroll the connection in Vault's database secrets engine and returns
  `JSON.stringify({ type: 'vault', connectionId })` as `encryptedCredentials`; otherwise encrypts via
  `packages/secrets` as before. Both paths are mutually exclusive per-connection and coexist in the same
  deployment — old connections continue using the envelope path until re-enrolled.
...
**Credential resolution (SQ-044).** `ClientFactory` is now async: `(target, role?) → Promise<{ client,
revokeOnDone }>`. `pg-client.ts`'s `defaultClientFactory` inspects `target.encryptedCredentials` —
if it parses as `{ type: 'vault', connectionId }`, it calls `vaultClient.mintCredentials(connectionId,
role)` (role defaults to `'read'`; `handleExecuteWrite` passes `'write'` to get the write-capable Vault
role) and the returned `revokeOnDone` calls `vaultClient.revokeLease(leaseId)` in the handler's
`finally` block for immediate post-execution revocation. If the credential is not a Vault ref, it falls
back to `decryptDatabaseCredentials(..., CREDENTIAL_MASTER_KEY)` with a no-op `revokeOnDone`. The Vault
client singleton is lazily constructed once per process from env vars — zero cost when Vault is not
configured.

Env vars: `CREDENTIAL_MASTER_KEY` (64 hex chars — **never set this anywhere else**), `STATEMENT_TIMEOUT_MS`,
`LOCK_TIMEOUT_MS`, `DEFAULT_ROW_CAP`, `VAULT_ADDR` (optional), `VAULT_TOKEN` (optional).
29 unit tests (`pnpm --filter @repo/tre-executor test`, vitest) — 22 original + 7 new Vault path
tests covering both the secrets and Vault branches of `handleTestConnection` and the Vault failure case.
...
**Worker-thread sharp edge — resolved (SQ-062).** The previously-documented tsx loader issue (spawning
`execute-write-worker.ts` as a `worker_threads` Worker failed because tsx's ESM hooks don't propagate
into spawned threads) is fixed by the Dockerize pass: `apps/tre-dispatcher/tsup.config.ts` now uses
`noExternal: [/^@repo\//]` on the `execute-write-worker` entry, which causes tsup/esbuild to inline the
full TypeScript source of `@repo/tre-executor`, `@repo/secrets`, `@repo/queue`, and `@repo/logger`
directly into `dist/execute-write-worker.js`. The compiled worker is completely self-contained — no
workspace symlinks or tsx hooks needed; only npm packages (`pg`, `pg-cursor`) remain external and are
resolved from the shared `node_modules`. The same `noExternal` rule applies to every other app's build
(`api`, `ai-service`, `tre-dispatcher`'s main entry), so `node dist/index.js` now runs correctly in any
Node 22 environment without tsx. Under `pnpm dev` the tsx path is unchanged (the `isTsSource` check in
`src/index.ts` still passes `--import tsx/esm` to the worker when running from `.ts` source).
...
| P3 | Real Isolation | 🟡 In progress. `execute_write` jobs run in their own `worker_threads` `Worker` (terminated after each job). Worker-thread tsx sharp edge **resolved** (SQ-062): `tsup.config.ts` added to api, ai-service, tre-dispatcher — `noExternal: [/^@repo\//]` bundles all workspace packages into compiled output, making `dist/execute-write-worker.js` fully self-contained. Multi-stage Dockerfiles for all 4 services + `.dockerignore` + `output: 'standalone'` for web. Vault added to `docker-compose.yml` (`hashicorp/vault:1.17`, dev mode). App services added with `profiles: [app]`. Per-org queue concurrency limiting (SQ-060) done. **Vault dynamic secrets (SQ-044) done**: `packages/vault-client` (native fetch, database secrets engine, 11 tests); `ClientFactory` made async + `revokeOnDone` for per-operation lease cleanup; `handleTestConnection` registers new connections with Vault when `VAULT_ADDR`/`VAULT_TOKEN` are set; all three execution handlers mint short-lived credentials per operation and revoke immediately post-execution; backward-compatible with existing envelope-encrypted connections. Still 🔲: actual container isolation (dockerode-launched write containers), k8s |
| P4 | Cloud-Native | 🟡 In progress. **GitHub Actions CI/CD (SQ-063) done**: `.github/workflows/ci.yml` (type-check + lint + test on every push/PR, Turborepo local cache via `actions/cache`) + `.github/workflows/docker.yml` (builds all 4 Docker images on main push, pushes to GHCR, runs Trivy CRITICAL/HIGH scan — gates push on failures, uploads SARIF to GitHub Security). **Helm chart (SQ-064) done**: `infra/k8s/safequery/` — Deployments for all 4 apps with `runAsNonRoot`, `readOnlyRootFilesystem`, `allowPrivilegeEscalation: false`, `capabilities.drop: [ALL]`; ClusterIP Services; HPA for api + web; NetworkPolicy (default-deny + allow-list: ai-service only reachable from api pods); Ingress (k3s Traefik, configurable class); Secrets template; values.yaml with full override surface. README updated (SQ-071). Still 🔲: Cloudflare edge (SQ-065) |
